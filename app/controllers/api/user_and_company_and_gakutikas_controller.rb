class Api::UserAndCompanyAndGakutikasController < ApplicationController
  ...
  private
    def correct_user
      ...
      # @user_and_company_and_gakutika = UserAndCompanyAndGakutika.eager_load(user_and_company: :user).find_by(id: params[:id])
      puts @user_and_company_and_gakutika.user_and_company.id == signin_user(request.headers).id
      render json: { message: ['該当する学チカが存在しません'] }, status: :bad_request if @user_and_company_and_gakutika.nil?
    end
end
